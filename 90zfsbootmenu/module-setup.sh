...
install() {
  ...
  # Install anything that is actually host-specific
  # shellcheck disable=SC2154
  if [ -z "${zfsbootmenu_release}" ] ; then
    ...
    # Install anything for a generic release
    echo -ne "\\x00\\x00\\x00\\x00" > "${initdir}/etc/hostid"
  fi
}
